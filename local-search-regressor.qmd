...
-   De schedules met gelijke objectives uit de grafiek halen, want het zijn geen errors. Nadenken over pre- en postprocessing van gelijkwaardige schedules.
-   Checken type neighbors voor training set.
...
def powerset(iterable, size=1):
    ...
    print(iterable)
    return [list(item) for item in chain.from_iterable(combinations(iterable, size) for r in range(len(iterable)+1))]
...
def get_neighborhood(x, v_star, ids):
    ...
    print(f'Neighborhood is {filtered_neighborhood}')
