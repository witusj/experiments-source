...
class CustomCallback(TrainingCallback):
    ...
    def after_iteration(self, model, epoch: int, evals_log: Dict[str, Any]) -> bool:
        if (epoch + 1) % self.period == 0:
            logger.info(f"Epoch {epoch}, Evaluation log: {evals_log['validation_0']['rmse'][epoch]}")
        return False
...
def main():
    ...
    save_best_trial_params(study, "best_regressor_trial_params.json")
    ...
    best_trial_params = load_best_trial_params("best_regressor_trial_params.json")
    logger.info(f"Loaded best trial parameters: {best_trial_params}")

    # Create an XGBoost regressor with the best parameters
    clf = xgb.XGBRegressor(**best_trial_params)
